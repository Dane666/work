...
# ----------------------------------------------------------------------------
# 净值回撤预警阈值（drawdown_watcher.py，风控辅助·净值事后审计）
# ----------------------------------------------------------------------------
# 每日收盘后对比模拟盘净值与历史峰值，回撤 = 当前净值/历史峰值 - 1（≤0）。
# 超阈值仅 Bark 推送提示，不修改策略/调仓逻辑；同级不重复推送（幂等）：
#   回撤 > -5%（未破关注线）             → ✅ 健康（不推送；自预警恢复时推送 ✅ 修复）
#   -10% < 回撤 ≤ -5%                  → ⚠️ 关注线
#   -15% < 回撤 ≤ -10%                 → 🚨 警戒线
#   回撤 ≤ -15%                        → 🔴 严重线（建议人工评估是否启动风控）
DD_WATCH_THRESHOLD = -0.05      # 关注线：-5%，触发 ⚠️
DD_ALERT_THRESHOLD = -0.10      # 警戒线：-10%，触发 🚨
DD_CRITICAL_THRESHOLD = -0.15   # 严重线：-15%，触发 🔴
DD_LEVEL_FILE = SIM_STATE_DIR / "last_dd_level.txt"   # 幂等标记：上次推送级别（随 data/state 持久化）
